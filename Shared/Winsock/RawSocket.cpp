...
void ASSERT(bool b_something)
{
    if (!b_something)
#ifdef _UNICODE
        OutputDebugString(L"Errror");
#else
...
void TypeWSAError(int i_error)
{
    if (i_error == WSAEWOULDBLOCK) LogMsg(_T("Winsock Error: WSAEWOULDBLOCK"));
    else
    if (i_error == WSAEINPROGRESS) LogMsg(_T("Winsock Error: WSAEINPROGRESS"));
    else
    if (i_error == WSAEALREADY) LogMsg(_T("Winsock Error: WSAEALREADY"));
    else
    if (i_error == WSAENOTSOCK) LogMsg(_T("Winsock Error: WSAENOTSOCK"));
    else
    if (i_error == WSAEDESTADDRREQ) LogMsg(_T("Winsock Error: WSAEDESTADDRREQ"));
    else
    if (i_error == WSAEMSGSIZE) LogMsg(_T("Winsock Error: WSAEMSGSIZE"));
    else
    if (i_error == WSAEPROTOTYPE ) LogMsg(_T("Winsock Error: WSAEPROTOTYPE"));
    else
    if (i_error == WSAENOPROTOOPT) LogMsg(_T("Winsock Error:WSAENOPROTOOP"));
    else
    if (i_error == WSAEPROTONOSUPPORT) LogMsg(_T("Winsock Error: WSAEPROTONOSUPPORT"));
    else
    if (i_error == WSAESOCKTNOSUPPORT ) LogMsg(_T("Winsock Error: WSAESOCKTNOSUPPORT"));
    else
    if (i_error == WSAEOPNOTSUPP) LogMsg(_T("Winsock Error: WSAEOPNOTSUPP"));

    else  LogMsg(_T("Winsock Error: Unknown."));
...
int	CWizReadWriteSocket::Read(void *pData, int nLen) 
{
	...
	do
		...
		if (r1 == SOCKET_ERROR)
            ...
            LogMsg(_T("RawSocket got blocked."));
            TypeWSAError(e);
            return 0;
			}
		else if (r1 == 0)
			return i_amount_read; 
		else if (r1 < 0)
			{
			ASSERT(0);
			return 0;
...
bool InitializeWinsock()
{
    ...
    {
        LogError(_T("Unable to init winsock."));
        return false;
    }
