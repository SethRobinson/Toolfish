...
#if _MSC_VER >= 1300
	return CWnd::CreateEx( dwExStyle, WC_TREEVIEW, NULL, dwStyle,
...
//Added by Seth A. Robinson
BOOL CTreeCtrlEx::IsChildNodeOf(HTREEITEM hitemChild, HTREEITEM hitemSuspectedParent)
{
	do
	{
...
//allows you to traverse a branch and get all items in it, including children.  h_parent is
//the branch.  You call this in a while(!null) style to get each thing, h_item needs to be
//what it last returned. (like an stl iterator)

HTREEITEM TraverseAllInBranch(CTreeCtrlEx & tree, HTREEITEM h_parent, HTREEITEM h_item)
{

    return tree.GetNextSiblingItem(h_item);
}



//Add by Seth A. Robinson Dec-28-2002
//returns false if it returns the root.  ("")
//This function uses reverse traversal to get you the string of where the item it is sent is stored.  For
//instance it could return "Produce\\Fruit\\Apples" if you sent it an item called "Green Apple" that was in the
//apples folder.

//This function assumes that you mark the data members of directories with a certain #, send that
//# with i_data_for_dir.  (This way if a standard item is sent over we won't take on it's name too)

bool GetBaseFolderOfItem(CTreeCtrlEx & tree, TCHAR *p_target, int i_data_for_dir, HTREEITEM h_item)
{
    _tcscpy(p_target, _T(""));
    //if the initial item is a dir, let's add it first.
    
    if (tree.GetItemData(h_item)  == DWORD(i_data_for_dir) )
    {
        _tcscat(p_target, _T("\\"));
        //add it, in reverse!
        CString cst_dir_name = tree.GetItemText(h_item);
        cst_dir_name.MakeReverse();
        _tcscat(p_target, cst_dir_name);
    }
    
    while (h_item = tree.GetParentItem(h_item))
    {
...
bool GetBaseFolderOfHighlightedItem(CTreeCtrlEx & tree, TCHAR *p_target, int i_data_for_dir)
{
    HTREEITEM h_item = tree.GetFirstSelectedItem();
    ...
    return GetBaseFolderOfItem(tree, p_target, i_data_for_dir, h_item);
}
