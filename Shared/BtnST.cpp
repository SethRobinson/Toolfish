...
//this define no longer needed when using VS8 it looks like
	...
	else
	{
		...
		else
		{
			...
			if (_tcslen(m_szURL) > 0)
			{
                ...
                if (!::ShellExecuteEx(&csSEI))
                {
                    TCHAR st_crap[256];
                    _stprintf(st_crap, _T("Shell excutefailed in the button class with error %d"), GetLastError());
                    OutputDebugString(st_crap);
                }
			} // if
		} // else
	} // else
...
DWORD CButtonST::SetBitmaps(TCHAR *p_bitmap_in, COLORREF crTransColorIn, TCHAR *p_bitmap_out, COLORREF crTransColorOut)
{
	HBITMAP		hBitmapIn		= NULL;
	HBITMAP		hBitmapOut		= NULL;
	
	// Find correct resource handle
	hBitmapIn = (HBITMAP)LoadImage(NULL, p_bitmap_in, IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
	
	
	// Load bitmap Out
	if (hBitmapOut)
    	hBitmapOut = (HBITMAP)LoadImage(NULL, p_bitmap_out, IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
...
DWORD CButtonST::SetBk(CDC* pDC)
{
    ...
    if (m_bDrawTransparent && pDC)
    {
        // Restore old bitmap (if any)
     if (m_dcBk.m_hDC != NULL && m_pbmpOldBk != NULL)
            //it's crashing on this line.. in GetSafeHandle?
        {
            m_dcBk.SelectObject(m_pbmpOldBk);
        } // if
